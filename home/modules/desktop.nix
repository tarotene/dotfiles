...
in
{
  ...
  # Darwin (ADR-0018): a much thinner slice — alacritty runs unwrapped (macOS
  # has its own native GL, no nixGL needed, ADR-0006), IME is the OS default
  # (no fcitx5/mozc on darwin), and fonts install via home.packages alone —
  # home-manager's targets/darwin copies fonts from the Nix store into
  # ~/Library/Fonts/HomeManager (symlinks are not recognized by Font Book),
  # so the Linux xdg.dataFile fontdb-seeding trick below has no darwin
  ...
  config = lib.mkMerge [
    (lib.mkIf pkgs.stdenv.isLinux {
      home.packages = [
        ...
        #
        # All three are wrapped for GL (#13 / ADR-0006). They tolerate a missing EGL
        # driver instead of dying the way alacritty does, which is exactly why it
        # went unnoticed: they just run their compositing on the CPU.
        #
        # Measured after wrapping, on company-pop-new:
        #   Chrome — GPU process holds 8 fds on /dev/dri/renderD128 and maps
        ...
        # Its closure is already pulled in by the wrappers above, so this is free.
    ...
    (lib.mkIf pkgs.stdenv.isDarwin {
      home.packages = [
        # Unwrapped: macOS has its own native GL stack (ADR-0006), so nixGL
        # is Linux-only and simply absent from darwin's pkgs (see flake.nix).
        pkgs.alacritty
        pkgs.nerd-fonts.fira-code
        pkgs.udev-gothic-nf
      ];
