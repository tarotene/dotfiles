...
{ pkgs, ... }:
{
  home.packages = with pkgs; [
    ...
    # (`cmd | freeze -o out.png`), so a "Before" state captured before a
    ...
    # already-lost pre-change state. Output must be PNG, not SVG — `gh
    # --attach` (>= 2.99.0, see the flake.nix overlay) only accepts images.
    charm-freeze
