...
これは `docs/codex-plan-review.md` の「第二次の非収束」（最終ラウンドに carry-over 判定の
...
`escalated` のチェックは上限判定より**前**に置く（`docs/codex-plan-review.md` の closer /
...
criterion にしない**。これは `docs/codex-plan-review.md` の「judge をシェルに置くのが要点。
...
（`docs/issue-index.md` の「嘘の正確な数字を出さない」と同じ立場）。
...
- **PASS 経路が到達可能であること**（`docs/codex-plan-review.md` の「第二次の非収束」と
