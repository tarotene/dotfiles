...
| Claude Code plan-review hook | `config/claude/{hooks,commands}` via `home.file` (`home/modules/claude.nix`) | stays literal — no `programs.*` module covers Claude Code hooks | Acceptance-convergent GitHub Copilot CLI review gate on ExitPlanMode: a read-only custom agent emits findings that the hook's shell+jq validator and judge verify, and the gate is "no BLOCKER/MAJOR left open" (max 2 rounds, fail-open; silent no-op without a copilot binary per ADR-0005). Rationale and operations: [`copilot-plan-review.md`](claude/copilot-plan-review.md). `~/.claude/settings.json` is Claude-Code-owned, so registration is an idempotent jq merge at activation, same constraint class as the fcitx5 profile. |
