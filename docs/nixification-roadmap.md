...
| Claude Code plan-review hook | `config/claude/{hooks,commands}` via `home.file` (`home/modules/claude.nix`) | stays literal — no `programs.*` module covers Claude Code hooks | Auto Codex review gate on ExitPlanMode (deny→revise, max 2 rounds, fail-open; silent no-op without a codex binary per ADR-0005). `~/.claude/settings.json` is Claude-Code-owned, so registration is an idempotent jq merge at activation, same constraint class as the fcitx5 profile. |
