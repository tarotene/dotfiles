...
## 「要確認」はブロッキング項目専用(#239 の指摘を機に明確化)

esa MCP のトークン供給を切り替えた PR(tarotene/dotfiles#239)で、マージ後の
残作業(登録の適用・動作確認・旧リポジトリの archive)を機械的に全部
「要確認」へ書いてしまい、ユーザーから「これは私がやることですか?
真にあなたにお任せできることはお任せして並行できることは並行して進めたい。
ブロッキングになるところだけ待ち合わせすればいいのでは」と指摘された。

指摘を受けて実際に仕分け直すと、真にブロッキング(ユーザーでなければ実行
できない)だったのは「外部サービスでの資格情報の発行」1件だけで、他はすべて
Claude 自身が(その場、またはマージ後に)実行できるものだった。加えて
「hms を実際に適用して動作確認する」は、PR 作成後であっても待つ理由がなく
即座に実行できた——実際に実行し、その結果を「検証」節に書き足して裏取りした。

この失敗は「要確認 = 人間にしかできない残作業のみ」というスキルの元々の
規定(SKILL.md 冒頭のスケルトン)に反していた。単に運用が追いついていな
かっただけなので、新しい概念は追加せず、SKILL.md 6節に「ブロッキング理由の
閉集合」(資格情報発行・ハードウェア操作・secrets 衛生・GUI操作・人間の
判断)と「今すぐ実行できるものは着手を待たない」という判定手順を明文化した。
閉集合の設計はこのリポジトリの ADR-0003(YubiKey 挿入・鍵の手動 import を
