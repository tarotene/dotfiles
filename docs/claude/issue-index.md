...
(`docs/claude/wrapup-inbox.md` と同じ設計原則)。
...
会社の社内リポジトリ D(Issues 機能を無効化している)は両方の Search が
...
- 会社の社内リポジトリ B の `@me` assign 68 件のうち **27 件(4 割)は他人起票**
...
ネットワークには出ない(`docs/claude/wrapup-inbox.md` と同じ流儀)。**未認証はここでは「対象外」
