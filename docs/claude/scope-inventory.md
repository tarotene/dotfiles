...
## 依頼に含まれない隣接事項の後続処理は stacked-pr スキルに委譲

要求インベントリ自体は「依頼に含まれていたのに落とした項目」だけを縛り、
依頼に含まれない隣接事項の非スコープ宣言までは縛らない(SKILL.md §4)。
ただし後続処理(stacked PR の一段にするか wrap-up inbox に流すか)の設計
根拠は本文書ではなく `docs/claude/stacked-pr.md`「スコープ外発見を stack
の一段として受ける入口」に置いた——判定の主体が `stacked-pr` スキルの
判定条件(§1)であり、要求インベントリ固有の論点ではないため。
...
このフォールバックは `tracking-issue` スキル導入(#145)以降、**この
dotfiles リポジトリ自身では発火しなくなっていく**見込みである(親 Issue の
子が GitHub ネイティブ sub-issues に一元化されるため、`totalCount == 0` に
なる場面が減る)。それでも削らず残しているのは、この hook が
`~/.claude/hooks` に user-wide 配備され、sub-issues 未導入の他リポジトリ
(dotfiles 以外)でもこの gate が働くため — フォールバックを削ると、その
リポジトリ群では経路Aの検査自体が丸ごと死ぬ(#147 の裁定)。
