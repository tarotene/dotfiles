...
——履歴改変の安全境界は既存の道具（会社側の別リポジトリで運用している専用の
rebase ツール）に委譲する。
...
会社側で運用している別のリポジトリの Issue は、まったく同じ「base 追従の
stale をどう扱うか」をすでに決着させている:
...
（特に会社の他リポジトリ群）ではスクリプト自体は無条件配備されているが、allowlist に無ければ
