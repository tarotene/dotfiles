...
allowed-tools: Bash(bash @home@/.claude/hooks/copilot-plan-review.sh --advisory *)
---
...
2. 次を実行する（数分かかる。タイムアウトは 300000 ms を指定）:
   ```
   bash @home@/.claude/hooks/copilot-plan-review.sh --advisory <プランファイルの絶対パス> <プロジェクトの絶対パス>
   ```
