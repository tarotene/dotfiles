...
open Issue が積み上がると、同じ出自(同一の設計判断・同一の先行 PR・同一の構想)から生まれた複数の Issue や、同一の運用領域に属し完了を一緒に追跡したい複数の Issue が、一覧上はただの兄弟として並んでしまい、依存関係や完了条件が見えなくなる。このスキルは、その構造を GitHub の機能で表現し直し、かつ「作っただけで放置され腐る tracking Issue」を作らないための定期手順を定式化する。
...
- 出自が異なっていても、同一の運用領域(例: リポジトリ基盤の保守)に属し、完了を一緒に追跡したい場合は束ねてよい。ただし「open Issue の数を減らしたいから」だけでは束ねる根拠にならない(§3 参照)— 出自同一クラスタとは異なり、この根拠で束ねた傘の本文には束ねた理由(運用領域の名前)を明記する
...
- 出自がばらばらでも、同一の運用領域に属し完了を一緒に追跡したいクラスタは 1 本の傘にまとめてよい(§1 参照)
- 逆に、束ねる意味のない停滞 Issue 群(出自もばらばらで、運用領域も共有せず、互いに無関係な chore 等)を無理に一つの傘にまとめない。束ねる意味があるのは「一緒に完了を追跡したいから」であって、「open Issue の数を減らしたいから」ではない
